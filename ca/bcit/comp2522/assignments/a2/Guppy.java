...
public class Guppy {
    ...
    /**
     * Spawns guppies.
     * @return babyGuppies as an array list.
     */
    public ArrayList<Guppy> spawn() {
        if (!isFemale || ageInWeeks < 8) {
            return null;
        } else {
            ArrayList babyGuppies = new ArrayList<Guppy>();
            Random random = new Random();
            double rollBabies = random.nextDouble();
            if (rollBabies <= 0.50) {
                int numOfBabies = random.nextInt(101);
                for (int i = 0; i < numOfBabies; i++) {
                    boolean newIsFemale = false;
                    double rollGender = random.nextDouble();
                    if (rollGender <= 0.5) {
                        newIsFemale = true;
                    }
                    babyGuppies.add(new Guppy(DEFAULT_GENUS, DEFAULT_SPECIES,
                            0, newIsFemale, 1 + getGenerationNumber(),
                            (1.0 + getHealthCoefficient()) / 2.0));
                }
            }
            return babyGuppies;
        }
